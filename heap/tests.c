...
void max_heap_tests() {
    ...
    { // inserting
        ...
        heap_insert(heap, 7);
        assert_array(heap->elements, mksa(0, 7, 5), 3);
        heap_insert(heap, 40);
        assert_array(heap->elements, mksa(0, 40, 5, 7), 4);
    }
    ...
    { // deleting
        ...
        assert_array(heap->elements, mksa(0, 35, 30, 25, 5, 15, 10), 7);
        assert(heap_delete(heap) == 35);
        assert_array(heap->elements, mksa(0, 30, 15, 25, 5, 10), 6);
        assert(heap_delete(heap) == 30);
        assert_array(heap->elements, mksa(0, 25, 15, 10, 5), 5);
...
void min_heap_tests() {
    ...
    { // inserting
        ...
        assert(heap->current_index == 2);
        heap_insert(heap, 7);
        assert_array(heap->elements, mksa(0, 5, 7), 3);
        heap_insert(heap, 40);
        assert_array(heap->elements, mksa(0, 5, 7, 40), 4);
    }

    { // top
        struct Heap* heap = create_min_heap(100);
        for (int i = 100; i >= 1; i--) heap_insert(heap, i);
        assert(heap_top(heap) == 1);
    }
    ...
    { // deleting
        ...
        for (int i = 0; i < 7; i++) heap_insert(heap, values[i]);
        assert(heap_delete(heap) == 5);
        assert_array(heap->elements, mksa(0, 10, 15, 25, 30, 35, 40), 7);
        assert(heap_delete(heap) == 10);
        assert_array(heap->elements, mksa(0, 15, 30, 25, 40, 35), 6);
        assert(heap_delete(heap) == 15);
        assert_array(heap->elements, mksa(0, 25, 30, 35, 40), 5);
        assert(heap_delete(heap) == 25);
        assert(heap_delete(heap) == 30);
        assert(heap_delete(heap) == 35);
        assert(heap_delete(heap) == 40);   
